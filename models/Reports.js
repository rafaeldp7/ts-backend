...
// Pre-save middleware to sync fields and auto-geocode
reportSchema.pre('save', async function(next) {
  ...
  // Sync location formats and ensure at least one format exists
  if (this.location) {
    ...
    // Ensure at least one coordinate format exists (for backward compatibility)
    if (!this.location.latitude && !this.location.coordinates?.lat) {
      // If neither exists, this will fail validation - but we allow it for flexibility
      // The application should validate before saving
    }
  }
