...
exports.updateMotorcycle = async (req, res) => {
  try {
    const { id } = req.params;
    const updatedMotor = await Motorcycle.findByIdAndUpdate(id, req.body, { new: true });
    if (!updatedMotor) return res.status(404).json({ msg: "Motorcycle not found" });

    res.status(200).json({ msg: "Motorcycle updated successfully", motorcycle: updatedMotor });
  } catch (error) {
    res.status(400).json({ msg: "Failed to update motorcycle", error: error.message });
  }
};
...
exports.deleteMotorcycle = async (req, res) => {
  try {
    const { id } = req.params;
    const deletedMotor = await Motorcycle.findByIdAndUpdate(id, { isDeleted: true }, { new: true });
    if (!deletedMotor) return res.status(404).json({ msg: "Motorcycle not found" });

    res.status(200).json({ msg: "Motorcycle soft-deleted successfully" });
  } catch (error) {
    res.status(500).json({ msg: "Failed to delete motorcycle", error: error.message });
  }
};
